...
void MemAccess(string op,int value,int eff){
    ...
    if (op == "LB") size = 1;
    else if (op == "LH") size = 2;
    else if (op == "LW") size = 4;
...
void PMI(int EA, int pc, int data, int ra, string action = "",string op = "")
{
    if (action == "write")
    {
        cout << "PMI Call; Writing " << data << " to memory address " << EA << endl;
        MemAccess(op,data,EA);
    }
    else if (action == "read")
    {
...
int main()
{
    ...
    while (global_pc / 4 < InstructionPCPairs.size())
    {
        ...
        { // S-type instruction (store operations)
            ...
            cout<<RegFile[stoi(info[2])]<<" "<<stoi(info[3])<<" "<<alu_output<<endl;
        ...
        { // S-type: store operation has already computed the effective address.
            PMI(alu_output, current_pc, RegFile[stoi(info[1])], 0, "write",info[0]);
            
        }
        else if (opcode == 0x13 && info[0] == "LW") // Example for load operations (if implemented)
        {
            PMI(alu_output, current_pc, 0, stoi(info[1]), "read",info[0]);
