...
int32_t parseImmediate(const string& immStr) {
    size_t pos = 0;
    int32_t imm;
    try {
...
int main() {
    ...
    input_file.open("input.asm",ios::in);
    ...
    try {
        ...
        if (info.type == "R") {
            ...
            uint8_t rd = parseRegister(tokens[1]);
            ...
            uint8_t rs2 = parseRegister(tokens[1]);
            string mem = tokens[2];
            ...
            int32_t imm = parseImmediate(mem.substr(0, lparen));
            ...
            uint8_t rs1 = parseRegister(tokens[1]);
