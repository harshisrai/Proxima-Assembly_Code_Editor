...
int Execute()
{
    ...
    if (branch_inst && !buffer2.is_unconditional) {
        if(buffer2.ra){
            ...
            if(it!=BTB.end()){ // it was there before. thus a prediction was made.
            ...
            else{ // First time seeing the conditional branch instruction
                                         ...
                                         { return st.instr != nullptr; }))
    {
     ...
     branchPrediction = false;
        ...
        if (pipeline[4].instr)
        {
            ...
            WriteBack();
            if(buffer3.rs1_needs_MEM_to_MEM || buffer3.rs2_needs_MEM_to_MEM)buffer3.mem_output = buffer4.mem_output;
